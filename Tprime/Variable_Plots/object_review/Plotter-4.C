...
/*ps      if (observable.Contains("mass")){
	dataCut = "((diphoMVA > -0.5) && (mass < 115 || mass > 135))";
        ...
        {*/
	dataCut = "weight*((dipho_leadIDMVA > -0.9) &&  lepton_leadPt>10 && weight>0 && abs(bjet1_eta)<2.4)";
/*          if (process.label == "QCD" || process.label == "GJet"){
...
*/  
	if(process.label == "TprimeM700"){
		mcCut = "weight*((dipho_leadIDMVA > -0.9) &&  lepton_leadPt>10 && weight>0 && abs(bjet1_eta)<2.4)"; // for thq
	}
	else mcCut = "weight*((dipho_leadIDMVA > -0.9) && (dipho_mass < 115 || dipho_mass > 135) &&  lepton_leadPt>10 && weight>0 && abs(bjet1_eta)<2.4)";
/*          }
...
//        TLatex * tex1 = new TLatex(0.10, 0.99,TString("#bf{CMS}"));
        ...
        TLatex * tex2 = new TLatex(0.70, 0.97,TString(Form("%.1f fb^{-1} (13 TeV)",lumi)));
        tex2->SetNDC();
...
TString output_dir= "/eos/user/p/prsaha/www/Tprime_analysis/DataVsMC/UL/2018/";   
