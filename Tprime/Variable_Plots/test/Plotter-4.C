...
//      TString treeName = "tagsDumper/trees/" + process.treePrefix + "_13TeV_"+mode;
      TString treeName = process.treePrefix + "_13TeV_"+mode;
...
/*ps      if (observable.Contains("mass")){
	dataCut = "((diphoMVA > -0.5) && (mass < 115 || mass > 135))";
        ...
        {*/
		dataCut = "(MVAscore_TprimeVsHiggs > 0.4 && (dipho_mass > 115 && dipho_mass < 135) && Tprime_mass>650 && Tprime_mass<1600)";
/*          if (process.label == "QCD" || process.label == "GJet"){
...
*/  
		...
		mcCut = "weight*(MVAscore_TprimeVsHiggs > 0.4 && (dipho_mass > 115 && dipho_mass < 135) && Tprime_mass>650 && Tprime_mass<1600)"; // for thq
		}
		else mcCut = "weight*(MVAscore_TprimeVsHiggs > 0.4 && (dipho_mass > 115 && dipho_mass < 135) && Tprime_mass>650 && Tprime_mass<1600)";
/*          }
...
//    leg = new TLegend(0.81,0.27,0.93,0.90);
	...
	if(process.label != "TprimeM700") leg->AddEntry(histos[procIndex], process.label/* + Form(" (%.1f)", histos[procIndex]->GetSum()) ,"f"*/);
    }
...
/*	TLatex * tex = new TLatex(0.10, 0.97,TString("CMS Preliminary    ") + 
	...
	tex->Draw();
*/
//        TLatex * tex1 = new TLatex(0.10, 0.99,TString("#bf{CMS}"));
...
TString output_dir= "/eos/user/p/prsaha/www/Tprime_analysis/Finalfit_Lite_output/UL/result_v2/";   
    canvas->SaveAs(output_dir + mode + "_" + observable + ".pdf");
    canvas->SaveAs(output_dir + mode + "_" + observable + ".png");
