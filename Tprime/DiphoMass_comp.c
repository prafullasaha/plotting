...
//    hist_NonHiggs[i] =new TH1F("MVA_score_NonHiggsBkg"," ;MVA_score_NonHiggsBkg", 100, -0.4, 0.4);
	...
	tree_-> GetEntry( ievent );
	    if(procs[i]  == "Tprime_600" || procs[i]  == "Tprime_625" || procs[i]  == "Tprime_650"|| procs[i]  == "Tprime_675" || procs[i]  == "Tprime_700"){
    	    cut = ((qTprime_mass>460 && qTprime_mass<800) && qMVA_HiggsBkg> 0.64);
    	}
    	else if(procs[i]  == "Tprime_800" || procs[i]  == "Tprime_900" || procs[i]  == "Tprime_1000"){
        	cut = ((qTprime_mass>650 && qTprime_mass<1300) && qMVA_HiggsBkg> 0.4);
    	}
    	else if(procs[i]  == "Tprime_1100" || procs[i]  == "Tprime_1200" ){
        	cut = ((qTprime_mass>650 && qTprime_mass<1600) && qMVA_HiggsBkg> 0.2);
    	}		
//cout<<"Cut ="<< cut <<endl;
	if(cut==1){
...
c2->SaveAs("/eos/user/p/prsaha/www/Tprime_analysis/DiphoMass_comp_AfterOpt.pdf");
c2->SaveAs("/eos/user/p/prsaha/www/Tprime_analysis/DiphoMass_comp_AfterOpt.png");
} 
