...
//    cout << "First try : (delta,solution)" << Delta << "," << solution << endl;
    if( Delta >= 0){
      ...
      soln_1++;	
      total_soln++;
//cout<<"[Check frac of soln] soln_1 count= "<<soln_1<<"/"<<total_soln<<endl;
      W_mass = mw;
      Soln_Tag= 1;
      return nut;
    }
    ...
    for (nLoopsToSolve = 1; nLoopsToSolve < 1001; nLoopsToSolve++) {
      mw = smearedWmass();
      solution = pZNeutrino(mw, mu, met , Delta);
//      if(nLoopsToSolve==1000)cout << nLoopsToSolve << " try : (smearedWmass,delta,solution) "<< mw << "," << Delta << "," << solution << endl;
      if (Delta >= 0 /*&& (mw < 0 || mw > 200)*/){
//	cout << nLoopsToSolve << " try : (smearedWmass,delta,solution) "<< mw << "," << Delta << "," << solution << endl;
//	if (abs(solution) > 2000){cout<<"Smeared mass for high pz  "<<mw<<endl;}
	nut.SetPxPyPzE(met.Px(), met.Py(), solution, sqrt(met.Px() * met.Px() + met.Py() * met.Py() + solution * solution));
	soln_2++;
//cout<<"[Check frac of soln] soln_2 count= "<<soln_2<<"/"<<total_soln<<endl;
	W_mass = mw;
	Soln_Tag = 2;
	return nut;
      }
...
//    cout << "Last result " << solution << endl;
    nut.SetPxPyPzE(met.Px(), met.Py(), solution, sqrt(met.Px() * met.Px() + met.Py() * met.Py() + solution * solution));
    soln_3++; 
//cout<<"[Check frac of soln] soln_2 count= "<<soln_3<<"/"<<total_soln<<endl;
    Soln_Tag =3;
    W_mass = mw;
    return nut;
  }
  ...
  float getW_mass()const {
    return W_mass;
  }

  float getSoln_Tag()const {
    return Soln_Tag;
  }
