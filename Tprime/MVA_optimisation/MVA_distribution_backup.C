...
string procs[file_size] = {"ttgg", "ttgjets","DiPhotonJetsBox","gjets","ttjets","tgjets"/*,"WW","WZ","ZZ"*/,"ttwjets","ZG","WG",/*"ttZ","tZq",*/"DYJets", Form("Tprime_%s",T_mass[0].c_str()), Form("Tprime_%s",T_mass[1].c_str()),"tth_125","thq_125","vh_125","Data"};
...
string procs_[file_size] = {"tt#gamma#gamma", "tt#gamma + jets","#gamma#gamma + jets","#gamma + jets","tt + jets","t#gamma+jets",/*"WW","WZ","ZZ",*/"TTW","Z + #gamma","W + #gamma",/*"ttZ","tZq",*/"Drell-Yan",Form("Tprime_%s",T_mass[0].c_str()), Form("Tprime_%s",T_mass[1].c_str()), "ttH","tHq","VH","Data"/*, "ttjets", "gjetsPt20to40","DYJets"*/};
...
TLegend *leg1 = new TLegend(0.2, 0.7, 0.89, 0.89, "");
	leg1->SetNColumns(4);
...
//                }
        ...
        if(procs[i] != Form("Tprime_%s",T_mass[0].c_str()) || procs[i] != Form("Tprime_%s",T_mass[1].c_str()) || procs[i] != "Data"){
...
cout<<"ttH Yield   ="<<hist_HiggsBkg->Integral()<<endl;
cout<<"tHq Yield   ="<<hist_Tprime2->Integral()<<endl;
hist_Data->SetBinErrorOption(TH1::kPoisson);
cout<<"hist_data->GetBinError(9) "<<hist_Data->GetBinError(9)<<endl;

for(int j=0; j<(file_size-6); j++)
{
	hist[j]->SetBinErrorOption(TH1::kPoisson);
	for(int bin=0; bin < hist[j]->GetNbinsX()+1; bin++){
		cout<<j<<"Hist with bin content  = "<<hist[j]->GetBinContent(bin)<<"\t"<<hist[j]->GetBinError(bin)<<endl;
...
for(int bin=0; bin < totalMC->GetNbinsX()+1; bin++){
...
for(int i=0 ; i<file_size; i++)
{
    if(procs[i]=="ttgg"){hist[i]->SetFillColor(kSpring-6); hist[i]->SetLineColor(kSpring-6); hs->Add(hist[i]); leg1->AddEntry(hist[i], procs_[i].c_str(),"f");}
    else if(procs[i]=="ttgjets"){ hist[i]->SetFillColor(kTeal+1);hist[i]->SetLineColor(kTeal+1); hs->Add(hist[i]); leg1->AddEntry(hist[i], procs_[i].c_str(),"f");}
	else if(procs[i]=="DiPhotonJetsBox"){ hist[i]->SetFillColor(kBlue); hist[i]->SetLineColor(kBlue); hs->Add(hist[i]); leg1->AddEntry(hist[i], procs_[i].c_str(),"f");}
	else if(procs[i]=="ttjets"){ hist[i]->SetFillColor(kSpring-9); hist[i]->SetLineColor(kSpring-9); hs->Add(hist[i]); leg1->AddEntry(hist[i], procs_[i].c_str(),"f");}
	else if(procs[i]=="gjets"){ hist[i]->SetFillColor(kCyan); hist[i]->SetLineColor(kCyan); hs->Add(hist[i]); leg1->AddEntry(hist[i], procs_[i].c_str(),"f");}
	else if(procs[i]=="Data"){leg1->AddEntry(hist_Data, procs_[i].c_str(),"ep"); }
	else if(procs[i]==Form("Tprime_%s",T_mass[0].c_str())){leg1->AddEntry(hist_Tprime1, Form("T'_%s ",T_mass[0].c_str()), "f"); }
    else if(procs[i]==Form("Tprime_%s",T_mass[1].c_str())){leg1->AddEntry(hist_Tprime2, Form("T'_%s ",T_mass[1].c_str()), "f"); }
    else if(procs[i]=="tth_125"){leg1->AddEntry(hist_HiggsBkg, "SM-Higgs ","f"); }
...
TLatex * tex1 = new TLatex(0.12, 0.97,TString("#bf{CMS} #it{work in progress}"));
	 tex1->SetNDC();
     ...
     TLatex * tex2 = new TLatex(0.77, 0.97,TString(Form("%.0f fb^{-1} (13 TeV)",lumi)));
     tex2->SetNDC();
...
if(BDT_Higgs){
	hRatio_stat_Err->GetYaxis()->SetRangeUser(0,2);
  	hRatio_stat_Err->GetXaxis()->SetTitle("BDT-SMH");
...
TPad *pad1 = new TPad("pad1","pad1",0,0.3,1.0,1.0);
	...
	hs->SetMaximum(100*hist_Data->GetMaximum());
	hs->SetMinimum(10e-4);
	}		
...
c1->SaveAs(outfile_path + Form("/%s_Score_Run%i_v1.root",method.c_str(),year));
c1->SaveAs(outfile_path + Form("%s_Score_Run%i_v1.png",method.c_str(),year));
c1->SaveAs(outfile_path + Form("%s_Score_Run%i_v1.pdf",method.c_str(),year));
