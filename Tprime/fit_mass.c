...
void fit_mass(TString object, TString T_mass, double init_mean, double lower_bound, double upper_bound, double init_sigma){
...
tree->Draw("Tprime_mass >> h_tmass", "weight", "hist");
...
RooRealVar  Tprime_mass("Tprime_mass", "Tprime_mass", lower_bound, upper_bound);
RooRealVar  mass("mass", "mass", lower_bound, upper_bound);
//RooDataHist data("data","dataset with T' mass", mass , h_mass) ;
//RooDataHist data("data","dataset with T' mass", Tprime_mass , h_mass) ;
RooDataSet data("data","dataset with T' mass", tree, Tprime_mass);//, "Tprime_mass > 0", "weight") ;


RooRealVar  mu("mu", "mu", init_mean, lower_bound, upper_bound);
RooRealVar  sigma("sigma", "sigma", init_sigma, 0.1, 100.);
RooGaussian gaus("gaus", "gaus", Tprime_mass, mu, sigma);
...
//RooGaussian gaus_2nd("gaus_2nd", "gaus_2nd", Tprime_mass, mu_2nd, sigma_2nd);
RooGaussian gaus_2nd("gaus_2nd", "gaus_2nd", Tprime_mass, mu, sigma_2nd);

RooRealVar  mu_3("mu_3", "mu_3", init_mean, lower_bound, upper_bound);
RooRealVar  sigma_3("sigma_3", "sigma_3", init_sigma, 0.1, 100.0);
//RooGaussian gaus_3("gaus_3", "gaus_3", Tprime_mass, mu_3, sigma_3);
RooGaussian gaus_3("gaus_3", "gaus_3", Tprime_mass, mu, sigma_3);

RooRealVar  mu_4("mu_4", "mu_4", init_mean, lower_bound, upper_bound);
RooRealVar  sigma_4("sigma_4", "sigma_4", init_sigma, 0.1, 100.0);
RooGaussian gaus_4("gaus_4", "gaus_4", Tprime_mass, mu, sigma_4);
//RooGaussian gaus_4("gaus_4", "gaus_4", Tprime_mass, mu, sigma_4);
...
RooGaussian model("model", "model", Tprime_mass, mu, sigma);

Tprime_mass.setRange("Range",init_mean-40,init_mean+40);

model.fitTo(data,Minos(true), NumCPU(8), RooFit::Range("Range"));
//gaus.fitTo(data,Minos(true), NumCPU(8), RooFit::Range("Range"));
cout<<"par 1=	-----------------------------------------------------------------"<<mu.getValV()<<"#pm"<<mu.getError()<<endl;
...
RooDataSet *pseudo_data = model.generate(Tprime_mass, 100 * 1000);
    TH1 *model_hist = pseudo_data->createHistogram("Tprime_mass", 100 * 10);
...
c1->SetTitle("");
c1->SetGridx();
RooPlot *frame_mass=Tprime_mass.frame();
frame_mass->SetTitle("");
...
//    latex.DrawLatex( 0.150, 0.85, Form("#scale[0.75]{Mean : %.2f GeV}", mode) );
    latex.DrawLatex( 0.150, 0.85, Form("#scale[0.75]{Mean : %.2f  #pm %0.2f GeV}", mu.getValV(), mu.getError()) );
	latex.DrawLatex( 0.150, 0.80, Form("#scale[0.75]{#sigma_{eff} : %.2f #pm %0.2f GeV}",sigma.getValV()/2, sigma.getError()/2) );
//	latex.DrawLatex( 0.77, 0.90, Form("  : %.2f #pm %.2f", mode_1, mode_sigma_1) );
//h_mass->Fit(double_gaus, "R");
//h_mass->Draw("");
//h_tmass->Draw("hist");
c1->SaveAs("/eos/user/p/prsaha/www/Tprime_analysis/UL/Tmass_fit_test/"+object+"_massOn_Tprime"+T_mass+".png");
c1->SaveAs("/eos/user/p/prsaha/www/Tprime_analysis/UL/Tmass_fit_test/"+object+"_massOn_Tprime"+T_mass+".pdf");
