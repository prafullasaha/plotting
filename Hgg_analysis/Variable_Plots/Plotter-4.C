...
//    TH1F *ratioPlot= NULL;
	...
	dataCut = "((diphoMVA > -0.5) && (mass < 115 || mass > 135))";
        ...
        {
          dataCut = "(dipho_leadIDMVA > -0.9)"; // for thq
...
cout<<"Hello, I am here"<<endl;
            mcCut = "weight*((dipho_leadIDMVA > -0.9 ))"; // for thq
          }
...
//        histo->Scale(lumi);
      ...
      if (! process.isData && normalized)
	{
	  ...
	  dataYield = dataHisto->GetEntries();
    ...
    if(ratio){
      ...
      TPad *pad1 = new TPad("pad1","pad1",0,0.3,0.9,1.0);
...
//	  double dataMax = dataHisto->GetMaximum() + sqrt(dataHisto->GetMaximum());
	  ...
	  double theMax = 1.2 * max(dataMax, hs2->GetMaximum()); // new stack
//	  dataHisto->SetMaximum(1.2*dataMax);
      ...
      if (dataHisto != NULL) 
	{
      ...
      TPad *pad2 = new TPad("pad2","pad2",0,0.05,0.9,0.3);
      pad2->SetTopMargin(0);
...
//      ratioPlot->GetYaxis()->SetRange(-5,5); 
	...
	hRatio->Divide(totalMC);
      ...
      hRatio->GetYaxis()->SetTitle("#frac{tHq}{Bkg}");
      hRatio->GetXaxis()->SetTitle(observable);
    ...
    if (dataHisto != NULL)
      leg->AddEntry(dataHisto,TString("tHq")/* + Form(" (%d)", int(dataHisto->GetEntries() + 0.5))*/,"l");
    ...
    canvas->SaveAs(mode + "_" + observable + ".pdf");
    canvas->SaveAs(mode + "_" + observable + ".png");
