...
hSignal->Draw();
      ...
      else
	{
      ...
      // add to THStack
      if (! process.isData && process.label != "tHq X 200")
	hs1->Add(histo);
...
//	  double dataMax = dataHisto->GetMaximum() + sqrt(dataHisto->GetMaximum());
	  ...
	  double theMax = 5 * max(dataMax, hs2->GetMaximum()); // new stack
...
*/
        ...
        TLatex * tex2 = new TLatex(0.5, 0.99,TString(Form("%.0f fb^{-1} (13 TeV)",lumi)));
        tex2->SetNDC();
...
TString dir= "/eos/user/p/prsaha/www/Hgg_analysis/Variable_plots/DataVsMC/";   
    canvas->SaveAs(dir + mode + "_" + observable + ".pdf");
    canvas->SaveAs(dir + mode + "_" + observable + ".png");
