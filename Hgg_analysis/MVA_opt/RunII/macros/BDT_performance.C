...
void BDT_performance(){
	...
	Float_t bweight, bdipho_mass, bMVA1, bMVA2;
        ...
        tree_tth->SetBranchAddress("MVAscore_tHqVsttH_BDT", &tMVA1);
//        tree_tth->SetBranchAddress("MVA_thqVsNonPeakingBkg", &tMVA2);
        ...
        tree_thq->SetBranchAddress("MVAscore_tHqVsttH_BDT", &qMVA1);
//        tree_thq->SetBranchAddress("MVAscore_tHqVsNonHiggsBkg", &qMVA2);
	...
	tree_bkg->SetBranchAddress("MVAscore_tHqVsttH_BDT", &bMVA1);
...
int N=0;
s_sig->SetTitle("#bf{#frac{s}{#sqrt{s+b}} Vs DNN score(ttHVstHq)}; #bf{DNN score}; #bf{Significance}");
tHq_eff->SetTitle("Efficiency Vs DNN score(ttHVstHq); #bf{DNN score}; #bf{Efficiency}");
ttH_eff->SetTitle("Efficiency Vs DNN score(ttHVstHq); #bf{DNN score}; #bf{Efficiency}");
bkg_eff->SetTitle("Efficiency Vs DNN score(ttHVstHq); #bf{DNN score}; #bf{Efficiency}");
mg->SetTitle("#bf{Efficiency Vs DNN score(ttHVstHq)}; #bf{DNN score}; #bf{Efficiency}");
	for(double MVA1=-0.3;MVA1 <= 0.3;MVA1=MVA1+0.01){
	cout<<"Debug[2]"<<endl;
N++;
cout<<"N =  "<<N<<endl;
	float s=0;
...
cout<<"MVA1=  "<<MVA1<<"s_sig_   ="<<s_sig_<<endl;
s_sig->SetPoint(N, MVA1, s_sig_);
tHq_eff->SetPoint(N, MVA1, tHq_eff_);
ttH_eff->SetPoint(N, MVA1, ttH_eff_);
bkg_eff->SetPoint(N, MVA1, bkg_eff_);
}
...
eff->SaveAs("BDT_eff.pdf");
eff->SaveAs("BDT_eff.png");
