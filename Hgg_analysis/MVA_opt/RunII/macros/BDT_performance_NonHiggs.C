...
float MVA1_cut=0.3;
float lumi = 137.5;
	for(double MVA2=-0.3;MVA2 <= 0.3;MVA2=MVA2+0.2){
	cout<<"Debug[2]"<<endl;
N++;
cout<<"N =  "<<N<<endl;
	float s=0;
...
                }
	...
	cout<<"MVA2=  "<<MVA2<<"s_sig_   ="<<s_sig_<<endl;
...
TLine *eff_l = new TLine(MVA1_cut,0,MVA1_cut,100);
eff_l->SetLineWidth(3);
eff_l->SetLineColor(13);
eff_l->SetLineStyle(7);

TLine *sig_l = new TLine(MVA1_cut,0,MVA1_cut,0.3);
...
eff_l->Draw();
tex->Draw();
eff->SaveAs("plots/BDT_eff_NonHiggs.root");
sig->SaveAs("plots/BDT_sig_NonHiggs.root");
}
