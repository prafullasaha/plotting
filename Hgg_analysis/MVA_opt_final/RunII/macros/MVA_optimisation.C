...
cout<<"N =  "<<N<<endl;
	...
	double s_sig_wrtTTH_ = Getsig(s,b);
        ...
        cout<<"MVA2=  "<<MVA2<<"s_sig_wrtBkg_   ="<<s_sig_wrtBkg_<<"   tHq eff= "<<tHq_eff_<<"   ttH eff"<<ttH_eff_<<endl;
...
s_sig_wrtTTH->SetTitle(Form("; #bf{%s score}; #bf{Significance}", method.c_str()));
s_sig_wrtBkg->SetTitle(Form("; #bf{%s score}; #bf{Significance}", method.c_str()));
...
TLegend *leg1 = new TLegend(0.7, 0.7, 0.9, 0.9, "");
    ...
    leg1->SetBorderSize(0);
        ...
        TLatex * tex = new TLatex(0.10, 0.97,TString("#bf{CMS}                  ") + Form("%.1f", lumi) + " fb^{-1} (13 TeV)");
        tex->SetNDC();
        tex->SetTextAlign(13);
        tex->SetTextFont(42);
...
//l->Draw("SAME");
tex->Draw();
eff->SaveAs(Form("/eos/user/p/prsaha/www/tmp/MVA_opt/%s_eff_Run%iBDTcut1st_v1.root",method.c_str(), year));
sig_wrtTTH->SaveAs(Form("/eos/user/p/prsaha/www/tmp/MVA_opt/%s_sig_wrtTTH_Run%iBDTcut1st.root",method.c_str(), year));
sig_wrtBkg->SaveAs(Form("/eos/user/p/prsaha/www/tmp/MVA_opt/%s_sig_wrtBkg_Run%iBDTcut1st.root",method.c_str(), year));

eff->SaveAs(Form("/eos/user/p/prsaha/www/tmp/MVA_opt/%s_eff_Run%iBDTcut1st_v1.pdf",method.c_str(), year));
sig_wrtTTH->SaveAs(Form("/eos/user/p/prsaha/www/tmp/MVA_opt/%s_sig_wrtTTH_Run%iBDTcut1st.pdf",method.c_str(), year));
sig_wrtBkg->SaveAs(Form("/eos/user/p/prsaha/www/tmp/MVA_opt/%s_sig_wrtBkg_Run%iBDTcut1st.pdf",method.c_str(), year));

eff->SaveAs(Form("/eos/user/p/prsaha/www/tmp/MVA_opt/%s_eff_Run%iBDTcut1st_v1.png",method.c_str(), year));
sig_wrtTTH->SaveAs(Form("/eos/user/p/prsaha/www/tmp/MVA_opt/%s_sig_wrtTTH_Run%iBDTcut1st.png",method.c_str(), year));
sig_wrtBkg->SaveAs(Form("/eos/user/p/prsaha/www/tmp/MVA_opt/%s_sig_wrtBkg_Run%iBDTcut1st.png",method.c_str(), year));
return 0;
}
