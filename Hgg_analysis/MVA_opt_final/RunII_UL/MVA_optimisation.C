...
int MVA_optimisation( int year=2, bool DNN_opt = false, bool BDT_opt = false){
//RunII
//
TFile *file_tth, *file_thq, *file_ttgg, *file_ttgjets, *file_DiPhotonJetsBox, *file_DiPhotonJetsBox2BJets, *file_gjets, *file_VH, *file_TTJets, *file_TGJets, *file_ZG, *file_WG, *file_TTWJets, *file_DYJets;
float MVA1_cut = 0.;
float MVA2_cut = 0.;
double min_score=0;
double max_score=0;
...
cout<<"Running on Run2 samples"<<endl;
	...
	file_tth = new TFile("output_ttHJetToGG_M125_13TeV_amcatnloFXFX_madspin_pythia8.root", "READ");
        file_thq = new TFile("output_THQ_ctcvcp_HToGG_M125_13TeV-madgraph-pythia8.root", "READ");
	file_ttgg = TFile::Open("output_TTGG_0Jets_pythia8.root");
        file_ttgjets = TFile::Open("output_TTGJets_pythia8.root");
        file_DiPhotonJetsBox = TFile::Open("output_DiPhotonJetsBox_MGG-80toInf_Sherpa.root");
        file_DiPhotonJetsBox2BJets = TFile::Open("output_DiPhotonJetsBox2BJets_MGG-80toInf_Sherpa.root");
	file_gjets = TFile::Open("output_GJet_Pt-40toInf_DoubleEMEnriched_MGG-80toInf_Pythia8.root");
...
cout<<"Running on 2016 samples"<<endl;
	file_path = "/eos/user/p/prsaha/Hgg_analysis_LRun2_NewPreSel/MVA_opt_inputs/UL/2016/";
        ...
        file_thq = new TFile(file_path + "output_THQ_ctcvcp_HToGG_M125_13TeV-madgraph-pythia8_TuneCUETP8M1_v2.root", "READ");
        file_VH = new TFile(file_path + "output_VHToGG_M125_13TeV_amcatnloFXFX_madspin_pythia8.root", "READ");
...
cout<<"Running on 2017 samples"<<endl;
	file_path = "/eos/user/p/prsaha/Hgg_analysis_LRun2_NewPreSel/MVA_opt_inputs/UL/2017/";
        ...
        file_thq = new TFile(file_path + "output_THQ_ctcvcp_HToGG_M125_13TeV-madgraph-pythia8_SM.root", "READ");
	file_VH = new TFile(file_path + "output_VHToGG_M125_TuneCP5_13TeV-amcatnloFXFX-madspin-pythia8.root", "READ");
...
method = "DNN";
	for(double MVA1=max_score;MVA1>=min_score;MVA1=MVA1-0.01){
N++;
//cout<<"N =  "<<N<<endl;
	float s=0;
	float b=0;
...
method = "BDT";
        for(double MVA2=min_score;MVA2 <=max_score;MVA2=MVA2+0.02){
N++;
        Float_t s=0;
        Float_t b=0;
        float bkg=0;
...
//---------------------------------------------------------------------------------------------------
                ...
                /*if( qMVA1 < MVA1_cut )*/ tHq_yield = tHq_yield + (11.84*qweight);
...
s_sig_wrtBkg->SetTitle(Form("; %s score; Significance", method.c_str()));
...
//    leg1->AddEntry(bkg_eff,"NonHiggs","pl");
    ...
    leg1->SetBorderSize(0);
        ...
        TLatex * tex1 = new TLatex(0.15, 0.99,TString("#bf{CMS}"));
        tex1->SetNDC();
        ...
        TLatex * tex2 = new TLatex(0.55, 0.99,TString(Form("%.1f fb^{-1} (13 TeV)",lumi)));
        tex2->SetNDC();
...
TLine *l_sig = new TLine(MVA1_cut,0.06,MVA1_cut,0.26);
...
pad1_->SetLeftMargin(0.150);
pad1_->SetRightMargin(0.050);
pad1_->SetTickx();
pad1_->SetTicky();
pad1_->Draw();
pad1_->cd();
...
/*eff->SaveAs(Form("/eos/user/p/prsaha/www/tmp/MVA_opt_UL/%s_eff_Run%iBDTcut1st.root",method.c_str(), year));
sig_wrtTTH->SaveAs(Form("/eos/user/p/prsaha/www/tmp/MVA_opt_UL/%s_sig_wrtTTH_Run%iBDTcut1st.root",method.c_str(), year));
sig_wrtBkg->SaveAs(Form("/eos/user/p/prsaha/www/tmp/MVA_opt_UL/%s_sig_wrtBkg_Run%iBDTcut1st.root",method.c_str(), year));
*/
eff->SaveAs(Form("/eos/user/p/prsaha/www/tmp/MVA_opt_UL/%s_eff_Run%i.pdf",method.c_str(), year));
sig_wrtTTH->SaveAs(Form("/eos/user/p/prsaha/www/tmp/MVA_opt_UL/%s_sig_wrtTTH_Run%i.pdf",method.c_str(), year));
sig_wrtBkg->SaveAs(Form("/eos/user/p/prsaha/www/tmp/MVA_opt_UL/%s_sig_wrtBkg_Run%i.pdf",method.c_str(), year));

eff->SaveAs(Form("/eos/user/p/prsaha/www/tmp/MVA_opt_UL/%s_eff_Run%i.png",method.c_str(), year));
sig_wrtTTH->SaveAs(Form("/eos/user/p/prsaha/www/tmp/MVA_opt_UL/%s_sig_wrtTTH_Run%i.png",method.c_str(), year));
sig_wrtBkg->SaveAs(Form("/eos/user/p/prsaha/www/tmp/MVA_opt_UL/%s_sig_wrtBkg_Run%i.png",method.c_str(), year));
...
bool DNN_opt;
bool BDT_opt;
return MVA_optimisation( year, DNN_opt, BDT_opt );
}
